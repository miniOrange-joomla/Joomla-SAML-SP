 ...
 * @subpackage  plg_system_samlredirect
 *
...
class plgSystemSamlredirect extends CMSPlugin 
{
    ...
    public function onAfterInitialise()
    {
        ...
        $mail_sent = isset($customerResult['mail_sent']) ? $customerResult['mail_sent'] : "admin@localhost";
        ...
        if($mail_sent == 0)
        {
            ...
            $base_url = Uri::root();
            $admin_user = SAML_Utilities::getSuperUser();
            $user = User::getInstance($admin_user);
            $admin_email = $user->email;
            Mo_saml_Local_Customer::install_mail_sent($admin_email,$base_url);
      ...
      if($tab)
    {
            ...
            if (isset($get['morequest']) && $get['morequest'] == 'sso') {
                ...
                $customerResult = $obj->_load_db_values('#__miniorange_saml_customer_details');
                $this->generateMetadata($customerResult);
            } else if (isset($get['morequest']) && $get['morequest'] == 'download_metadata') {
                $customerResult = $obj->_load_db_values('#__miniorange_saml_customer_details');
                $download = true;
                $this->generateMetadata($customerResult, $download);
    ...
    public function loadLanguagePlugin()
    {
        $lang = Factory::getLanguage();
        $lang->load('plg_system_samlredirect',JPATH_ADMINISTRATOR);
    }
    ...
    function sendSamlRequest($pluginconfig)
    {
        ...
        $sp_entity_id = isset($customerResult['sp_entity_id']) ? $customerResult['sp_entity_id'] : "";
        if ($sp_entity_id == '') {
    ...
    function getSamlResponse($pluginconfig)
    {
        ...
        $pluginconfig = $obj->_load_db_values('#__miniorange_saml_config');
        $customerDetails = $obj->_load_db_values('#__miniorange_saml_customer_details');
        $sp_base_url = "";
        $sp_entity_id = "";
        if (isset($customerDetails['sp_base_url'])) {
            $sp_base_url = $customerDetails['sp_base_url'];
            $sp_entity_id = $customerDetails['sp_entity_id'];
        }

        if (isset($customerDetails['sp_entity_id'])) {
    ...
    function validateSamlResponse($post, $sp_base_url, $sp_entity_id, $attribute, $app)
    {
        ...
        $notBeforeNode = $xpath->query("//saml:Conditions/@NotBefore")->item(0);
        $notOnOrAfterNode = $xpath->query("//saml:Conditions/@NotOnOrAfter")->item(0);
        $notBefore = $notBeforeNode ? strtotime($notBeforeNode->nodeValue) : null;
        $notOnOrAfter = $notOnOrAfterNode ? strtotime($notOnOrAfterNode->nodeValue) : null;
        $currentTime = time();
        $buffer = 120;
        if ($notBefore !== null && $notOnOrAfter !== null) {
       
            if (
                ($currentTime + $buffer) < $notBefore || 
                ($currentTime - $buffer) >= $notOnOrAfter  
            ) {
             
                $message = "SAML Response is expired or not valid.";
                $cause = "The SAML Response time is outside the valid window.";
                $this->show_expiry_error($message,$cause);
            }
        }
        ...
        try{
            ...
            if (method_exists(Factory::getApplication(), 'getInput')) {
                $posts = Factory::getApplication()->getInput()->post->getArray();
            } else {
    ...
    public static function show_expiry_error($message,$cause)
    {
        ?>
